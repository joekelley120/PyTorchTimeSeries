...
def init_tdl(model, inputs, labels):
    ...
    # input tap-delay
    itdl = inputs[:, max_delay_size - model.input_delay_size: max_delay_size, :]

    # output tap-delay
    otdl = labels[:, max_delay_size - model.output_delay_size: max_delay_size, :]
...
def init_tdl_zeros(model, batch_size=1):
    ...
    # input tap-delay
    itdl = torch.zeros((batch_size, model.input_delay_size, model.input_size), dtype=torch.float64)

    # output tap-delay
    otdl = torch.zeros((batch_size, model.output_delay_size, model.output_size), dtype=torch.float64)
...
def train_scg(model, input_tensor, label_tensor, epochs=1000, print_every=50, plot_every=False,
    ...
    """
    Train ARX model for k-steps ahead using Scaled Conjugate Gradient (SCG).
    ...
    for epoch in range(epochs + 1):
        ...
        def closure():
            ...
            scg.zero_grad()
            inputs, labels, itdl, otdl = init_tdl(model, batch_inputs, batch_labels)
...
def init_training_data(model, inputs, labels, prediction_horizon=1, loading_length=0, overlap=True):
    ...
    :param loading_length: time length used for loading the ARX
...
def prediction_mae(model, input_tensor, label_tensor, loading_length=0, device=None):
    ...
    inputs, labels, itdl, otdl = init_tdl(model, input_tensor, label_tensor)
    ...
    # Combine loading and k-step predictions/labels
...
def prediction_mse(model, input_tensor, label_tensor, loading_length=0, device=None):
...
def prediction_rmse(model, input_tensor, label_tensor, loading_length=0, device=None):
...
def unscaled_prediction_mae(model, input_tensor, label_tensor, scalar, loading_length=0, device=None):
...
def unscaled_prediction_mse(model, input_tensor, label_tensor, scalar, loading_length=0, device=None):
...
def unscaled_prediction_rmse(model, input_tensor, label_tensor, scalar, loading_length=0, device=None):
...
def plot_response(model, input_tensor, label_tensor):
...
def print_mse(model, input_tensor, label_tensor, k_step=[1]):
    ...
    :param input_tensor: input tensor
    :param label_tensor: output tensor
    :param k_step: k-step prediction for mse calculation (Default 1)
    :return: none
    """
    ...
    for z, k in zip(range(len(k_step)), k_step):
        ...
        err = (np.array(e) ** 2).sum() / er.size()[1]
        print('Step: ' + str(k) + ', MSE: ' + str(err))
...
def plot_error_histogram(model, input_tensor, label_tensor, k_step=[1], number_of_bins=25, limits=None):
    ...
    :param limits: limits for x-axis (lower, upper0) (Default None)
    :return: none
    """
...
def plot_skyline_response(model, points, max_width, min_width, max_height, min_height):
    ...
    # Forward pass through network
    itdl, otdl = init_tdl_zeros(model)
