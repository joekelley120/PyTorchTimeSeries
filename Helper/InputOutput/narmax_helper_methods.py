...
def init_tdl(model, inputs, labels, device):
    ...
    # error tap-delay
    etdl = torch.zeros((inputs.size()[0], model.error_delay_size, model.output_size), dtype=torch.float64).to(device)

    # input and label data
    input = inputs[:, max_delay_size:, :]
    label = labels[:, max_delay_size:, :]

    return input, label, itdl, otdl, etdl
...
def init_tdl_zeros(model, batch_size, device):
    ...
    # error tap-delay
    etdl = torch.zeros((batch_size, model.error_delay_size, model.output_size), dtype=torch.float64).to(device)

    return itdl, otdl, etdl


def train_scg(model, input_tensor, label_tensor, loading_length=0, epochs=1000, print_every=50, plot_every=False,
              number_batches=1, device=None):

    """
    Train NARMAX model for k-steps ahead using Scaled Conjugate Gradient (SCG).
    ...
    for epoch in range(epochs + 1):
        ...
        def closure():
            ...
            scg.zero_grad()
            inputs, labels, itdl, otdl, etdl = init_tdl(model, batch_inputs, batch_labels, device)
            ...
            loading_outputs, itdl, otdl, etdl = model.loading(loading_inputs, loading_labels, itdl, otdl, etdl)

            # Perform a k-step ahead prediction
            k_step_outputs, _, _, _ = model(k_step_inputs, itdl, otdl, etdl)

            # Create weights for performance
            perf_loading = criterion(loading_labels, loading_outputs)
            perf_k_step = criterion(k_step_labels, k_step_outputs)
            perf = perf_k_step + perf_loading
...
def init_training_data(model, inputs, labels, prediction_horizon=1, loading_length=0, overlap=True):
    ...
    tap_delay_size = max(model.input_delay_size, model.output_delay_size, model.error_delay_size)
...
def __multi_step_ahead_prediction(model, input_tensor, label_tensor, loading_length, device):
    ...
    inputs, labels, itdl, otdl, etdl = init_tdl(model, input_tensor, label_tensor, device)
...
def plot_errors(model, input_tensor, label_tensor, k_step=[1], loading_length=0):
...
def plot_acf(model, input_tensor, label_tensor, k_step=[1], lags=25, loading_length=0):
...
def plot_error_histogram(model, input_tensor, label_tensor, k_step=[1], loading_length=0, number_of_bins=25):
    ...
    :param label_tensor: output tensor
    :param k_step: plot acf at specified k-step prediction (Default 1)
    :param loading_length: time length used loading NARMAX
...
def plot_ccf(model, input_tensor, label_tensor, k_step=[1], lags=25, loading_length=0):
    ...
    inputs, labels, itdl, otdl, etdl = init_tdl(model, input_tensor, label_tensor, device)
    loading_labels, k_step_labels = labels[:, :loading_length, :], labels[:, loading_length:, :]
    loading_inputs, k_step_inputs = inputs[:, :loading_length, :], inputs[:, loading_length:, :]
...
def plot_skyline_response(model, points, max_width, min_width, max_height, min_height):
    ...
    itdl, otdl, etdl = init_tdl_zeros(model, batch_size=1, device=device)
    outputs, _, _, _ = model(input_tensor, itdl, otdl, etdl)
