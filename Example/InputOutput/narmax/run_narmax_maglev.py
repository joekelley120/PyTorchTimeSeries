...
NARMAX_SAVE_LOC = 'train/maglev/'
NARMAX_LOAD_LOC = NARMAX_SAVE_LOC + 'narmax_td6_n10'
...
def main():
    ...
    # NARMAX TDL Sizes for Input, Output, Errors
    delays = 6
    ...
    for i in [100]:
        ...
        if not trained_model:
            ...
            print()
            print("Training k-Step Ahead NARMAX for Horizon: " + str(i))
            print("-------------------------------------------------------")
            ...
            model, _ = train_scg(model, input_tensor, label_tensor, epochs=epochs, loading_length=loading_length,
                                 print_every=print_every, plot_every=False, number_batches=1)

            # Save Model
            folder_name = NARMAX_SAVE_LOC + "narmax_" + CURRENT_TIME
            file_name = "narmax_model_" + str(i) + '.ph'
...
def import_data(filename):
    ...
    return np.genfromtxt(filename, delimiter=',', dtype=np.float32)
